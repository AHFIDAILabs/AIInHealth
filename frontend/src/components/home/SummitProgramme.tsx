    ...
    <section className="bg-offwhite py-20">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <Reveal className="text-center">
          <h2 className="font-display text-2xl font-bold text-navy sm:text-3xl">{t('home.programme.heading', 'Summit Programme')}</h2>
        ...
        {sessions === null ? null : sessions.length === 0 ? (
          <Reveal delay={0.1} className="mt-10 rounded-2xl border border-slate-200 bg-white py-14 text-center">
            <p className="text-sm text-slate-500">{t('home.programme.empty', 'No sessions have been published yet. Check back soon!')}</p>
          </Reveal>
        ) : (
          <div className="mt-10 space-y-3">
            {sessions.map((s, i) => {
              const title = (lang !== 'en' && s.translations?.[lang]?.title) || s.title;
              return (
        ...
        <Reveal delay={0.15} className="mt-8 flex justify-center">
          <ButtonLink to="/agenda" variant="secondary" className="!border-slate-300 !bg-white !text-navy hover:!bg-offwhite">
            {t('home.programme.viewFull', 'View Full Agenda')}
          </ButtonLink>
        </Reveal>
      </div>
    </section>
