...
export const AbstractsPage = () => {
  ...
  const updateStatus = async (abstract: AdminAbstract, status: AbstractStatus) => {
    ...
    try {
      ...
      setItems((prev) => prev.map((a) => (a._id === abstract._id ? updated : a)));
      setActive(updated);
  ...
  const saveNotes = async () => {
    ...
    try {
      ...
      setItems((prev) => prev.map((a) => (a._id === active._id ? updated : a)));
      setActive(updated);
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div>
        <h1 className="font-display text-2xl font-semibold text-navy">Abstracts</h1>
        <p className="text-sm text-slate-500">{items.length} submission{items.length === 1 ? '' : 's'} for the Research track</p>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {ABSTRACT_STATUSES.map((s) => (
            <button
              ...
              className={`rounded-full border px-3.5 py-1.5 text-xs font-medium capitalize ${statusFilter === s ? 'border-orange bg-orange text-white' : 'border-slate-200 text-slate-600'}`}
            >
              {s}
            </button>
          ))}
                ...
                {items.map((a) => (
                  <tr key={a._id} onClick={() => openDetail(a)} className="cursor-pointer hover:bg-offwhite">
                    ...
                    <td className="px-3 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold capitalize ${STATUS_COLOR[a.status]}`}>{a.status}</span>
                    </td>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {ABSTRACT_STATUSES.map((s) => (
                      <button
                        ...
                        className={`rounded-full border px-3 py-1.5 text-xs font-semibold capitalize disabled:opacity-50 ${
